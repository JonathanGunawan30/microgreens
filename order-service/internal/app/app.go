...
func RunServer() {
	...
	if err != nil {
		log.Fatalf("[RunServer-3] Failed to register validator translations: %v", err)
	}
	...
	handler.NewOrderHandler(orderService, e, cfg, redisClient)
	...
	go func() {
		...
		if err != nil {
			log.Fatalf("[RunServer-4] Failed to start server: %v", err)
		}
	}()
	log.Info("[RunServer-5] Server is running on port ", cfg.App.AppPort)

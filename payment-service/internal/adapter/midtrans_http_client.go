...
func (m *midtransClient) CreateTransaction(orderID string, amount int64, customerName, customerEmail string) (string, error) {
	...
	transaction, err := snap.CreateTransaction(snapReq)
	if err != nil {
		log.Errorf("[MidtransClient] CreateTransaction: %v", err)
		return "", err
	}
